...
class FeaturedRepositories {
    ...
    constructor() {
        ...
        // Append To DOM
        const target: HTMLElement = document.getElementById('main-languages')!;
        ...
        getRepos().then((repos) => {
            repos.forEach((repository: RepoProperties) => {
                if (repository.topics.length == 0) return;

                const topics: string = repository.topics.join('|');

                if (this.target_keys.some((key: string) => topics.includes(key))) {
