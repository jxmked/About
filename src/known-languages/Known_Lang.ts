...
export default class Known_Lang {
     ...
     * */
    private COUNTED_LANGS: Record<string, number>;
        ...
        getRepos()
            .then((repos: RepoProperties[]) => {
                ...
                entries.forEach((item: [string, number]) => {
                    if (!skips.includes(item[0].toLowerCase())) rebuild[item[0]] = item[1];
                });
    ...
    createDOMElements(): void {
        ...
        const pert: Record<string, number> = this.COUNTED_LANGS;
