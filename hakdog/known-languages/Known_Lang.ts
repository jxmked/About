...
export default class Known_Lang {
  ...
  constructor() {
    ...
    // Add label
    Known_Lang.CONTAINER.appendChild(
      new Label({
    ...
    Known_Lang.PLACEMENT.parentNode!.insertBefore(
      Known_Lang.BASE,
      Known_Lang.PLACEMENT.nextSibling
    );
  }

  start(): void {
    getRepos()
      .then((repos: RepoProperties[]) => {
        ...
        const skips: string[] = (envRes.get('known-lang-skip') || []).map((x: string) =>
          x.toLowerCase()
        );
        ...
        entries.forEach((item: [string, number]) => {
          if (skips.indexOf(item[0].toLowerCase()) == -1) rebuild[item[0]] = item[1];
        });
        ...
        this.COUNTED_LANGS = Object.fromEntries(
          Object.entries(rebuild).map(([lang, count]) => {
        ...
        do {
          if (last && first) Known_Lang.CONTAINER.removeChild(last);
